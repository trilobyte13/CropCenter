...
/**
 * Tests for CropExporter.gridLinePixel — the package-private static helper that picks which output pixel each grid
 * line lands on for the baked-in export. The whole reason the function exists (instead of a plain
 * `Math.round((double) dim * i / count)`) is to keep `(i, count − i)` pairs spatially symmetric around `dim / 2`.
 * Java's Math.round is half-up, which would break that symmetry at exact half-integer positions — count=4, dim=10
 * produces raw values 2.5 and 7.5 that BOTH round half-up to 3 and 8 respectively, giving an asymmetric (3, 8)
 * pair around the midpoint at 5 instead of the symmetric (3, 7). The mirror trick (round one side, subtract from
 * dim for the other) fixes this. A regression that drops the trick produces visibly off-centre baked-in grids that
 * diverge from the preview, which the user sees as the grid lines "slipping" toward one edge of the cropped image.
 */
public final class CropExporterGridLineTest
{
	...
	public void singleLineGridProducesValidIndex()
	{
		...
		// position 6 (round half-up). Defensive — the count=2 case is degenerate but legal input from a rule-
		// of-thirds-style grid with a single internal divider.
