...
/**
 * Tests for the byte-array scanners on UltraHdrCompat. containsHdrgm is the only one that
 * runs without a Bitmap and is on the post-encode HDR-success verification path — a
 * regression that always returns true would mask a silent HDR-failed save with a false
 * "HDR OK" toast; one that always returns false would mark every HDR save as failed.
 */
public class UltraHdrCompatTest
{
	...
	{
		// Length checks: limit = length - 4, so any buffer < 5 bytes can't possibly hold
		// the 5-byte pattern. The loop bound has to handle these cleanly without throwing.
	...
	{
		// XMP namespace literals are exact-case in the spec — "HDRGM" or "Hdrgm"
		// should NOT match. A regression that case-folded would tag any file with
		// "HDRGM" in EXIF text fields as Ultra HDR.
	...
	public void containsHdrgmRejectsPartialMatches()
	{
		...
		assertTrue(UltraHdrCompat.containsHdrgm(
			"hdrgmm".getBytes(StandardCharsets.US_ASCII)));
		// These are too short or have a wrong byte mid-pattern.
		assertFalse(UltraHdrCompat.containsHdrgm(
			"hdrgxm".getBytes(StandardCharsets.US_ASCII)));
		assertFalse(UltraHdrCompat.containsHdrgm(
			"hdrg".getBytes(StandardCharsets.US_ASCII)));
		assertFalse(UltraHdrCompat.containsHdrgm(
			"drgm".getBytes(StandardCharsets.US_ASCII)));
	}

	@Test
	public void containsHdrgmScansEntireBufferNotJustPrefix()
	{
		// Earlier versions scanned only the first N bytes; a maxed-out EXIF thumbnail
		// can push the XMP segment past any fixed prefix window. Verify a hit deep in
		// a large buffer is still found.
