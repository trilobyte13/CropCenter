...
/**
 * Tests for ByteBufferUtils' read/write helpers and the overflow-safe bounds checks. The bounds checks are private but
 * exercised through every public read/write variant — driving them via the public API gives realistic call-shape
 * coverage and pins down the exact-fit boundary, the one-past-end rejection, the negative-offset rejection, and the
 * wraparound rejection at offsets near Integer.MAX_VALUE that the subtraction-based check rules out.
 */
public final class ByteBufferUtilsTest
{
	...
	public void readNegativeOffsetThrows()
	{
		...
		assertThrows(IndexOutOfBoundsException.class, () -> ByteBufferUtils.readU16BE(data, -1));
	}

	@Test
	public void readOnNullDataThrows()
	{
		assertThrows(IndexOutOfBoundsException.class, () -> ByteBufferUtils.readU16BE(null, 0));
	}
	...
	{
		// Subtraction-based check target. Under an `offset + length > data.length` form, `Integer.MAX_VALUE +
		// 2` wraps to a negative int that fails the bounds check by being < data.length, silently passing — and
		// then the array access at data[Integer.MAX_VALUE] AIOOBEs anyway, but with a less useful trace.
		// Subtraction-based check (`offset > data.length - length`) rejects up-front: MAX_VALUE > (8 - 2) = 6 →
		// throws.
		byte[] data = new byte[8];
		assertThrows(IndexOutOfBoundsException.class, () -> ByteBufferUtils.readU16BE(data, Integer.MAX_VALUE));
	}
	...
	public void readU16EndianDispatchPicksCorrectVariant()
	{
		...
		// writeU16AndWriteU32EndianDispatchPicksCorrectVariant test. ExifPatcher and BitmapUtils call readU16
		// with the boolean-dispatch form for orientation tags and IFD field counts; a regression that wired
		// both branches to the same native variant would silently mis-read every Samsung EXIF orientation tag
		// (the most common case being LE-encoded tags read as BE → garbage values).
		byte[] data = { 0x12, 0x34 };
	...
	public void readU16OnUndersizedBufferThrows()
	{
		...
		assertThrows(IndexOutOfBoundsException.class, () -> ByteBufferUtils.readU16BE(data, 0));
	}
	...
	public void readU32OnUndersizedBufferThrows()
	{
		...
		assertThrows(IndexOutOfBoundsException.class, () -> ByteBufferUtils.readU32LE(data, 0));
	}
	...
	{
		byte[] data = new byte[8];
		assertThrows(IndexOutOfBoundsException.class, () -> ByteBufferUtils.writeU32LE(data, -4, 0L));
	}
	...
	public void writeU16AndWriteU32EndianDispatchPicksCorrectVariant()
	{
		...
		// writeU16 / writeU32 to PATCH metadata in place — a regression that wired both branches to the same
		// native variant would silently corrupt every saved Ultra HDR file (MPF offsets land in the wrong byte
		// order, decoders refuse the gain map). Pin the dispatch directly.
		byte[] data = new byte[8];
	...
	{
		byte[] data = new byte[2];
		assertThrows(IndexOutOfBoundsException.class, () -> ByteBufferUtils.writeU16BE(data, 1, 0x1234));
	}
