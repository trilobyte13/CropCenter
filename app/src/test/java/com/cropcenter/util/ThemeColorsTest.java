 ...
 * res/values/colors.xml. The Java side exists for context-free callers (Paint setup, custom Views without
 * a Context); the XML side drives styles, layouts, and getResources().getColor lookups. A drift between
 * the two would manifest as a 1-frame color flash on first draw — EditorRenderer's onDraw clearColor would
 * disagree with the activity's window background — without any test failure or compile error.
 */
public final class ThemeColorsTest
{
	...
	public void overlayAndSubtextConstantMatchXmlValues()
	{
		...
		// EditorRenderer labels, AutoRotateBinder reset color). Both drive the same XML-vs-Java drift class
		// the rest of the constants in this file guard against — a single-side hex tweak would flash the
		// wrong color across multiple surfaces with no compile error.
	...
	public void surfaceConstantsMatchXmlValues()
	{
		...
		// EditorRenderer info bar / RotationRulerView major ticks (SURFACE2). Same drift hazard as the
		// accent and text constants above.
