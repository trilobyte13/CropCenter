...
/**
 * Tests for the cycle-and-overflow guards in ExifPatcher.scanIfd. Two adversarial EXIF
 * shapes pin down the recursion bound (cyclic SubIFD pointer) and the long-arithmetic
 * cast guard (SubIFD offset that overflows int when added to tiffStart). Both bypass-
 * the-guard regressions would corrupt the EXIF segment by writing orientation = 1 over
 * arbitrary bytes — the safest crash mode is a benign no-op, which is what the guards
 * deliver.
 */
public class ExifPatcherTest
{
	// Position of the TIFF header inside the EXIF segment payload, mirroring
	// ExifPatcher.TIFF_HEADER_OFFSET (FF E1 + 2-byte length + "Exif\0\0").
	private static final int TIFF_HEADER_OFFSET = 10;

	@Test
	public void patchTerminatesOnCyclicSubIfdChain() throws IOException
	{
		// IFD0 with two entries: a SubIFD (0x8769) pointing back at IFD0 itself
		// (offset 8 == IFD0's offset within the TIFF region), and an Orientation tag
		// (0x0112) so we can verify scanIfd actually ran. Without the depth guard,
		// scanIfd recurses unboundedly and throws StackOverflowError — uncatchable
		// in the calling Exception handlers, which would crash the bg thread.
		...
		List<JpegSegment> patched = ExifPatcher.patch(
			Collections.singletonList(exifSeg), 100, 200, null);

		// Orientation byte should be rewritten to 1. Address of the orientation value
		// = TIFF_HEADER_OFFSET + 8 (IFD0 offset relative to TIFF) + 2 (count) + 12 (skip
		// SubIFD entry) + 8 (skip tag/type/count of orientation entry).
	...
	{
		// SubIFD offset = 0xFFFFFFFF (u32 max). Without the long-arithmetic guard, the
		// expression `(int) (TIFF_HEADER_OFFSET + 0xFFFFFFFFL)` truncates to a small
		// positive int (TIFF_HEADER_OFFSET - 1 = 9) which falls inside the buffer — the
		// recursion would re-enter scanIfd on the EXIF header bytes themselves and
		// rewrite arbitrary positions. With the guard, the recursive call is skipped.
		// We verify the byte at the would-be-recursion target is unchanged.
		byte[] ifd = buildIfd(new int[][] {
		...
		// Snapshot the byte at TIFF_HEADER_OFFSET + 9 (the target of the truncated
		// cast). Without the guard scanIfd would write a u16 orientation value at
		// (target + 8) — clobbering EXIF header bytes around offset 17. Capture
		// pre-patch bytes 17..18 so we can verify they're unchanged.
	...
	{
		// Six-deep SubIFD chain: IFD0 → IFD1' → IFD2' → IFD3' → IFD4' → IFD5'. Depth
		// guard caps recursion at 4, so IFD5' is reached but its SubIFD pointer is
		// not followed. No exception, no infinite loop, and the legal-depth IFDs all
		// get their orientation tag rewritten.
		...
		// IFD4' (depth 4) should also be rewritten — the guard kicks in at depth > 4,
		// so depth=4 is still processed.
	...
	{
		// EXIF byte-order field is 2 bytes — "II" (little) or "MM" (big). A malformed
		// "IM" half-pair must not be silently treated as little-endian (which would
		// then parse subsequent u32 offsets with wrong byte order, possibly wandering
		// into the wrong corner of the segment buffer). Patch should fall through and
		...
		List<JpegSegment> patched = ExifPatcher.patch(
			Collections.singletonList(seg), 100, 200, null);

		assertEquals(1, patched.size());
		// Bytes must be identical to input — no orientation rewrite happened because the
		// byte-order rejection skipped scanIfd entirely.
	...
	{
		// Symmetric counterpart of patchReturnsSegmentUnchangedOnImByteOrder. A
		// malformed "MI" half-pair would be treated as big-endian if the validator
		// only checked the second byte; both halves must be checked.
		byte[] ifd = buildIfd(new int[][] {
			{ 0x0112, 3, 1, 0x0006 },
		});
	...
	public void maxThumbnailBytesReturnsBaselineForValidByteOrder() throws IOException
	{
		...
		// segment data layout: FF E1 LL LL + (Exif\0\0 + II*\0 + 4-byte ifd0Off + 18-byte
		// IFD0) = 4 + 6 + 4 + 4 + 18 = 36 bytes. Budget = 65535 - 36 - 42 = 65457.
	...
	{
		// The rejection branch (continue → fall through to "return
		// DEFAULT_THUMB_BUDGET" at end of loop) should fire for "IM". Without the
		// fix, maxThumbnailBytes would have parsed the segment with isLittleEndian=
		// true (only first byte 'I' checked) and returned a measured value
		// — distinct from DEFAULT_THUMB_BUDGET (20_000).
		byte[] ifd = buildIfd(new int[][] {
			{ 0x0112, 3, 1, 0x0001 },
		});
		JpegSegment seg = wrapAsExifSegmentWithByteOrder(ifd, (byte) 'I', (byte) 'M');
	...
	{
		// Non-EXIF segment passes through ExifPatcher.patch unchanged. Pinned because
		// the patcher's main loop has an early-continue for !seg.isExif() and a
		// regression that skipped the early-continue could clone every segment.
		...
		List<JpegSegment> patched = ExifPatcher.patch(
			Collections.singletonList(xmpSeg), 100, 200, null);

		assertEquals(1, patched.size());
		// Same instance reference — no clone for non-EXIF.
		assertFalse("non-EXIF segment should not have been cloned",
			patched.get(0).data() != xmpSeg.data());
	}

	/**
	 * Build an IFD body: 2-byte little-endian entry count + N * 12-byte entries +
	 * 4-byte next-IFD pointer (set to 0). Each entry is {tag, type, count, value}.
	 */
	...
	/**
	 * Wrap an IFD body as a complete EXIF JpegSegment: FF E1 + length + "Exif\0\0" +
	 * TIFF header (II*\0 + IFD0 offset = 8) + IFD body. Caller's IFD body sits at
	 * TIFF + 8 (i.e. payload offset TIFF_HEADER_OFFSET + 8).
	 */
	...
	/**
	 * Wrap an arbitrary TIFF region as an EXIF JpegSegment. Used for the multi-IFD
	 * chain test where the TIFF body holds several IFDs.
	 */
	...
	/**
	 * Like wrapAsExifSegment but lets the caller pick the 2-byte TIFF byte-order
	 * marker. Used for byte-order-rejection tests; pass ('I','M') or ('M','I') to
	 * exercise the validation branch.
	 */
	private static JpegSegment wrapAsExifSegmentWithByteOrder(byte[] ifd,
		byte high, byte low) throws IOException
	{
		ByteArrayOutputStream tiff = new ByteArrayOutputStream();
		tiff.write(high);
		tiff.write(low);
