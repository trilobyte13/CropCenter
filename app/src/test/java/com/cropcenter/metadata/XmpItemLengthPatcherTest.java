...
public final class XmpItemLengthPatcherTest
{
	...
	public void patchAcceptsSegLenAtTheU16FieldMax() throws IOException
	{
		...
		// unnecessarily reject two valid sizes. Pin that a patch growing the segment to within 65535
		// (the u16 segLen-field max) succeeds. Build a body sized so old segLen sits
	...
	{
		// When the first standard XMP segment carries an unpatchable Item:Length
	...
	public void patchHonorsFillBytesBeforeExtendedXmpFailClosed() throws IOException
	{
		...
		// Extended XMP chunk is preceded by a fill byte (`FF FF E1 ...`) and carries Item:Length in its
		// body. The walker must route through skipFillBytes so the fail-closed gate fires. A regression
		// to direct `primary[off+1]` reads here would let the stale Item:Length ship and silently
		// truncate HDR boost — the very leak the fail-closed paths exist to prevent. Pins
		// extendedXmpContainsItemLength's walker.
	...
	{
		// Legal JPEG fill bytes (extra 0xFF before the marker code, ITU-T T.81
	...
	{
		// When the first standard XMP segment patches successfully, the loop
	...
	public void patchRefusesWhenSegLenFieldWouldExceedU16Max() throws IOException
	{
		...
		// (a fall-through-to-input-unchanged behaviour would ship stale Item:Length when Extended XMP
		// didn't carry the pattern; the patcher distinguishes "not present" from "present but
		// unpatchable" and signals the caller to drop HDR).
		String tag = "<rdf:Description Item:Length=\"10\"/>";
		int xmpHeaderLen = JpegSegment.XMP_HEADER.length();
		int fillerLen = 65531 - xmpHeaderLen - tag.length();
		StringBuilder filler = new StringBuilder(fillerLen);
	...
	{
		// Body's final 12 bytes are exactly "Item:Length=" with no following
	...
	{
		// Body opens with `"` and closes with `'`. The patcher's
	...
	{
		// Body ends mid-digit-run (no closing quote). The patcher's
	...
	{
		// When standard XMP carries Item:Length= followed by something other than
	...
	{
		// Negative gainMapSize is a caller bug guarded by an early return,
	...
	{
		// Null primary is the documented input-validation fall-through.
	...
	{
		// When GContainer Item:Length lives in an Extended XMP chunk (the >64KB
	...
	{
		// When the 12-byte "Item:Length=" pattern straddles an Adobe Extended XMP chunk boundary, the
		// per-chunk substring scan misses it but a reassembly-based scan catches it. Without the
		// fallback, the patcher would return primary unchanged and the file would ship with stale
		// Item:Length, defeating the Extended-XMP fail-closed guard. Build a 2-chunk Extended XMP that
	...
	{
		// Reciprocal of patchSucceedsWhenStandardXmpHasItemLengthEvenWithExtendedXmpAlongside. Standard
		// XMP has no Item:Length; Extended XMP exists alongside but also doesn't carry Item:Length. Both
		// Extended-XMP scans (per-chunk + reassembly) must
	...
	{
		// Non-conformant source with TWO standard XMP APP1 segments. The first
