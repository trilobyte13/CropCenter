...
/**
 * Pins the JPEG marker constants against ITU-T T.81 / ISO/IEC 10918-1. These constants are the second byte of each
 * FF-prefixed marker pair (e.g. SOI is FF D8, so SOI = 0xD8) and are referenced by every JPEG byte-walker in the
 * codebase. A regression that flipped one would mis-route the entire marker walk and produce malformed JPEG output.
 */
public final class JpegMarkerTest
{
	...
	{
		// APP0..APP15 spans FF E0..FF EF (16 markers). The walkers iterate every APP-marker via `marker >= APP0
		// && marker <= APP_LAST`. A regression that flipped APP1 from 0xE1 to 0xE2 would silently route every
		// EXIF reader through the MPF marker code path; the existing walker tests pass byte fixtures, so a
		// constant-drift bug wouldn't surface there. Pin every value the codebase uses plus the 16-marker range
		// invariant. APP3 is the low boundary of GraftWriter.isVendorApp's APP3..APP15 range — a drift from
		// 0xE3 would silently widen or narrow which markers get stripped from grafted output.
	...
	{
		// PREFIX = 0xFF introduces every JPEG marker pair (FF MM). COM = 0xFE (FF FE) is the comment marker.
		// Both are referenced directly by test fixtures and byte-walkers; a regression in either would
		// mis-route the entire marker walk.
