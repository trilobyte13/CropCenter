 ...
 * (HORIZONTAL or VERTICAL lock mode) should be rejected because setCenter's joint clamp pushed the locked axis.
 * Pin the lock-axis selection (HORIZONTAL locks Y, VERTICAL locks X — the counter-intuitive naming the
 * production callers depend on), the strict 0.5px threshold, and the fall-through behaviour for BOTH / LOCKED /
 * null lock modes. A regression that flipped which axis is locked would let single-axis drags drift the wrong
 * axis on rotated images; a regression to >= 0.5 would falsely reject sub-pixel jitter from setCenter's clamp.
 */
public final class CropEditorViewCrossAxisDriftedTest
{
	...
	{
		// Strict greater-than: drift of exactly 0.5 px is NOT a violation. Pin so a regression to >= 0.5
		// would surface here as a false-positive that rejects sub-pixel drags.
	...
	{
		// HORIZONTAL lock = "drag locked to horizontal axis" = Y is the LOCKED axis (only X can move).
		// A Y movement of more than 0.5 px from the pre-drag position trips the drift check. The
		// post-X value is irrelevant — only Y is checked under HORIZONTAL lock.
	...
	{
		// CenterMode.LOCKED corresponds to Pan mode (the entire crop is fixed). The drift check is
		// irrelevant in Pan mode (no drag happens), but the function defends with false rather than NPE.
	...
	{
		// Defense-in-depth: null lock returns false rather than NPE. Production callers shouldn't pass
		// null but pin the contract.
	...
	{
		// The threshold check uses Math.abs — drift of -1.0 (post < pre) trips as much as +1.0 (post >
		// pre). Pin symmetry; a regression that dropped Math.abs would only catch positive drifts.
	...
	{
		// VERTICAL lock = "drag locked to vertical axis" = X is the LOCKED axis (only Y can move). A
		// post-X value drifting more than 0.5 px from pre-X is a violation.
