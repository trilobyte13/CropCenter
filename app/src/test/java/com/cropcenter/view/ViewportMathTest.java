 ...
 * fitToView / clampViewport / panViewport / zoomAt are exercised directly so a Bitmap-less CropState isn't required
 * either. The View-bound + CropState-bound overloads are thin adapters; testing the int-dimension variants exercises
 * every contract the production callers depend on.
 *
 ...
 *   - zoomAt: focus point stays under finger across zoom; zoom clamped to [1, maxZoom() = MAX_PIXEL_RATIO (64)
 *     / baseScale]; post-zoom clamp fires
 */
public final class ViewportMathTest
{
	...
	{
		// When the image is smaller than the visible window on an axis, that axis is centered on the image
		// center. Image 100x100, view 400x800, scale=1 → visibleW=400, visibleH=800. Both visibles exceed the
		// image dims; both axes center on imgW/2 = imgH/2 = 50.
		ViewportMath math = standardMath();
	...
	{
		// fitToView always picks baseScale so the constrained axis has visibleDim == imgDim (the centered
		// branch). To exercise the pin-to-bounds branch we have to zoom in past fit so visibleDim < imgDim.
		// Setup: 1000x2000 image in 400x800 view → baseScale = 0.4, viewportX starts at imgW/2 = 500. zoomAt(2x
		// at view center) doubles zoom → scale = 0.8, viewportX stays at 500 (focus-stationary identity). Now
		// visibleW = 400/0.8 = 500 < imgW = 1000, so the legal clamp range is [visibleW/2, imgW - visibleW/2] =
		// [250, 750]. Pan an absurd dx to drive viewportX past 0, then verify the post-pan clamp pinned it to
		// the LOWER bound (250), not the upper.
		ViewportMath math = standardMath();
	...
	public void clampViewportPinsToRotatedAabbBoundsWhenRotated()
	{
		...
		// un-rotated [0, 1000] × [0, 2000] rectangle on both axes. A clamp against the un-rotated bitmap would
		// pin viewportX into [visibleW/2, imgW - visibleW/2] = [250, 750], making the rotated corners (which
		// sit at screen-aligned X far outside [0, 1000]) unreachable at zoom > 1. The rotation-aware clamp must
		// permit viewportX inside [midX - rotatedW/2 + visibleW/2, midX + rotatedW/2 - visibleW/2] ≈ [-183,
		// 1183]. Pin both the left- and right-pan extremes.
		ViewportMath math = standardMath();
		math.fitToView(1000, 2000, 30f);
		math.zoomAt(2f, 200f, 400f, 1000, 2000, 30f);
		// scale after fit+2x zoom: baseScale × 2. visibleW = size.width() / scale. The exact lower bound is
		// midX - rotatedW/2 + visibleW/2; compute it from the same formula the clamp uses to pin the contract
		// regardless of the float rounding.
	...
	public void clampViewportPureOverloadDelegatesFromCropStateOverloadWithNullImage()
	{
		...
		// before/after and confirming no change. The int-dimension overload would rewrite viewportX/Y on this
		// input; the State-bound version SKIPS that rewrite.
		ViewportMath math = standardMath();
		float xBefore = math.screenToImageX(200f);
		math.clampViewport((CropState) null);
	...
	{
		// Pin that the ViewSize abstraction is actually load-bearing — same image-x = 0 should map to different
		// screen coords for different view sizes. A regression that hard-coded a dimension somewhere would
		// surface here.
	...
	public void effectiveMinZoomAtRotationDropsBelowOneWhenBaseScaleSetForUnrotatedImage()
	{
		...
		// Square 1000x1000 at 30°: rotatedDim = 1000*(cos30 + sin30) = 1366.025. expected min zoom = 1000 /
		// 1366.025 ≈ 0.7321
		float expected = (float) (1.0 / (Math.cos(Math.toRadians(30)) + Math.sin(Math.toRadians(30))));
		assertEquals("rotated min zoom drops below 1 to fit the rotated AABB", expected, minZoom30, TOL);
	}

	@Test
	public void effectiveMinZoomAtZeroRotationReturnsStaticMinZoom()
	{
		// Zero rotation collapses to the static MIN_ZOOM = 1, so user behavior at the common unrotated case is
		// unchanged. A regression that introduced a baseScale-relative formula at rotation=0 would shift the
		// zoom-out floor in the user's default workflow.
		ViewportMath math = standardMath();
		math.fitToView(1000, 2000);
	...
	public void effectiveMinZoomNeverExceedsOne()
	{
		...
		// baseScale was fit to) shouldn't raise minZoom above 1 — that would prevent the user from zooming in
		// past the fit-to-rotated baseScale. Pin the upper-bound enforcement directly.
		ViewportMath math = standardMath();
	...
	@Test
	public void effectiveMinZoomReturnsMinZoomOnNanRotation()
	{
		// The defensive second-pass guard's only reachable trigger: a NaN rotation propagates through cos/sin
		// into NaN rotated dims, and the `!(a > 0 && b > 0)` form folds NaN into the fail-closed branch. A
		// cleanup to `rotatedDims[0] <= 0 || rotatedDims[1] <= 0` lets NaN through — the division below then
		// returns a NaN zoom floor that Math.clamp propagates into the zoom on the next pinch: blank / frozen
		// viewport with no crash to point at.
		ViewportMath math = standardMath();
		math.fitToView(1000, 2000);
		assertEquals("NaN rotation must fail closed to MIN_ZOOM",
			1f, math.effectiveMinZoom(1000, 2000, Float.NaN), 0f);
	}
	...
	{
		// CropState overload's null-state branch returns MIN_ZOOM directly without touching baseScale — pin so
		// the production callers (zoomAt with state) don't have to re-check null.
		ViewportMath math = standardMath();
		math.fitToView(1000, 2000);
		assertEquals(1f, math.effectiveMinZoom((CropState) null), 0f);
	}
	...
	public void fitToViewBaseScaleTakesMinAcrossNonSquareImageAndView()
	{
		...
		// 800/1000=0.8. min is 0.1 (width-constrained). Pin so a regression that flipped min for max would zoom
		// too far in and lose the bottom of the image off-screen.
		ViewportMath math = standardMath();
	...
	{
		// CropState-bound overload short-circuits on null state. We can't construct a CropState with a loaded
		// Bitmap on a JVM, so verify the short-circuit directly: a null state must NOT alter baseScale / zoom /
		// viewport from their construction defaults.
		ViewportMath math = standardMath();
		float baseScaleBefore = math.getBaseScale();
		float zoomBefore = math.getZoom();
		math.fitToView((CropState) null);
	...
	public void fitToViewWith45DegreeSquareImageFitsRotatedBoundingBox()
	{
		...
		// 1/sqrt(2) ≈ 0.7071. A regression that uses the unrotated dims (baseScale = 1.0) lets the rotated
		// image extend past the view's edges — the literal bug fixed by adding rotation awareness to the fit
		// math.
		FixedViewSize size = new FixedViewSize(1000, 1000);
		ViewportMath math = new ViewportMath(size);
	...
	{
		// General contract: for any rotation, the rotated bounding box (widthForFit, heightForFit) must fit
		// inside the view dims after scaling by baseScale. Tests several non-trivial angles on a non-square
		// image to catch axis-swap bugs.
		int imgW = 4000;
		int imgH = 3000;
	...
	{
		// Sub-epsilon rotation (< BitmapUtils.ROTATION_EPSILON = 0.005°) is treated as zero by the rest of the
		// render pipeline, so the fit math collapses to the unrotated branch — produces the same baseScale as
		// the 2-arg overload exactly. Without the collapse a 0.001° rotation would shave a
		// negligible-but-nonzero fraction off baseScale, causing visible-but-inexplicable zoom drift on every
		// double-tap.
	...
	{
		// Two instances at the same dimensions and default state produce identical conversions for identical
		// inputs. Pin so a regression that stored state in a static / shared place surfaces here (different
	...
	{
		// With a non-trivial rotation (90°) and a default state where state.getImageWidth/Height return 0 (no
		// source bitmap), the rotation center collapses to the view center. A point at image-coord (0, 0) maps
		// to screen-center (viewW/2, viewH/2) = (200, 400) before rotation; rotating around the view center is
		// the identity, so the post-rotation output is still (200, 400). Pin this so a regression that rotated
		// around (0, 0) screen instead of around the image center would surface.
		ViewportMath math = standardMath();
		CropState state = new CropState();
		state.setRotationDegrees(90f);
		float[] out = new float[2];
		math.imageToScreenRotatedInto(0f, 0f, state, out);
		// Pre-rotation: (200, 400). Image center = (0, 0) image-coord → also (200, 400) screen. Rotating (200,
		// 400) around (200, 400) is identity → (200, 400) post-rotation.
	...
	{
		// At 90° rotation around the view center (= image center when no bitmap loaded), image-coord (100, 0) →
		// pre-rotation screen (200 + 100, 400 + 0) = (300, 400) (delta from center = (100, 0)). RotationMath's
		// CW convention rotates (dx, dy) = (100, 0) to (cos·dx - sin·dy, sin·dx + cos·dy) at θ=90° → (0, 100).
		// Final screen position = (200, 500). Pin this exact transform.
		ViewportMath math = standardMath();
		CropState state = new CropState();
		state.setRotationDegrees(90f);
		float[] out = new float[2];
	...
	{
		// Pin: null state must NOT NPE. The contract is "null state == zero rotation == identity branch". A
		// regression that dereferenced state without a null check would NPE here; one that fell through to the
		// rotation branch would still NPE on state.getImageWidth().
		ViewportMath math = standardMath();
		float[] out = new float[2];
	...
	{
		// Pin the alias contract — the function returns the SAME reference passed in `out`, not a fresh array.
		// onDraw relies on this to avoid per-frame allocation; a regression to `return new float[]{...}` would
		// re-introduce GC churn the renderer specifically avoids.
		ViewportMath math = standardMath();
		float[] out = new float[2];
	...
	{
		// Below the ROTATION_EPSILON snap threshold, the function uses the un-rotated screen coords directly
		// (no trig). Pin so the renderer's "treat tiny rotations as zero" agreement holds; a regression that
		// snapped rotation differently between math and renderer would cause tap-mapping drift while the image
		// is being drawn straight. ROTATION_EPSILON is 0.005°, so 0.001° is well below the threshold.
		ViewportMath math = standardMath();
		CropState state = new CropState();
	...
	{
		// At any (baseScale, zoom, viewportX, viewportY) state, screenToImageX(imageToScreenX(ix)) must equal
		// ix within float precision. Without a CropState we can only test the no-rotation path (no CropState
		// means no rotation applied). Pin the identity for several (ix, iy) values across the default viewport
		// state (zoom = 1, baseScale = 1, viewport = 0/0).
		ViewportMath math = standardMath();
	...
	{
		// At default state (viewportX = 0, scale = 1), the image-x=0 maps to view center = viewWidth / 2. Pin
		// so a regression that flipped the "center on viewport" semantics surfaces.
		ViewportMath math = standardMath();
	...
	{
		// Slope check: imageToScreenX(ix + 1) - imageToScreenX(ix) is a constant for any ix (= the scale). Pin
		// so a regression that introduced a quadratic / non-linear term in the mapping surfaces here.
		ViewportMath math = standardMath();
	...
	@Test
	public void maxZoomReturnsMinZoomOnZeroAndNanBaseScale()
	{
		// Degenerate baseScale is reachable before layout: fitToView against a 0-width view yields baseScale 0
		// (0 / rotatedW), and a 0-width view PLUS imgW = 0 hits the 0/0 division for NaN. The `!(baseScale >
		// 0f)` guard must floor BOTH to MIN_ZOOM — the direct `baseScale <= 0f` form silently lets NaN through,
		// and MAX_PIXEL_RATIO / NaN becomes a NaN zoom ceiling that Math.clamp propagates into every
		// screen-to-image conversion.
		ViewportMath zeroScale = new ViewportMath(new FixedViewSize(0, 800));
		zeroScale.fitToView(1000, 2000);
		assertEquals("baseScale 0 must floor maxZoom at MIN_ZOOM", 1f, zeroScale.maxZoom(), 0f);
		ViewportMath nanScale = new ViewportMath(new FixedViewSize(0, 0));
		nanScale.fitToView(0, 2000);
		assertEquals("NaN baseScale must floor maxZoom at MIN_ZOOM", 1f, nanScale.maxZoom(), 0f);
	}

	@Test
	public void maxZoomScalesInverselyWithBaseScale()
	{
		// Each source pixel renders at most MAX_PIXEL_RATIO (64) screen pixels regardless of source size — so a
		// small image (fit at baseScale ≈ 1) caps at zoom 64, while a large image (fit at baseScale ≈ 0.1) caps
		// at zoom 640. Pin both ends so a regression to a flat constant ceiling surfaces immediately.
		...
		assertEquals("small image (baseScale=1) caps at MAX_PIXEL_RATIO = 64", 64f, smallMath.getZoom(), TOL);
		...
		assertEquals("large image (baseScale=0.1) caps at 64 / 0.1 = 640", 640f, largeMath.getZoom(), TOL);
	}
	...
	{
		// State-bound overload skips ONLY the clamp on null state; the pan arithmetic still runs. Verify:
		// starting from default state (viewportX = 0, baseScale = 1, zoom = 1, scale = 1), pan dx=+10 should
		// move viewportX by -10. Reading screenToImageX(200) — the screen-center — should yield -10 (was 0
		// before; now viewportX = -10).
		ViewportMath math = standardMath();
		assertEquals("baseline: screen-center maps to viewportX = 0", 0f, math.screenToImageX(200f), 0f);
		math.panViewport(10f, 0f, (CropState) null);
	...
	public void screenToImagePixelIntoRoundTripsThroughImageToScreenRotatedInto()
	{
		...
		// precision. Pin so a regression in EITHER direction's rotation-center / sign / order would surface
		// here — both methods must agree on the same rotation center.
		ViewportMath math = standardMath();
		CropState state = new CropState();
	...
	public void screenToImagePixelIntoSubEpsilonRotationMatchesUnrotatedPath()
	{
		...
		// sub-epsilon snap prevents tap-mapping drift while the renderer draws the image straight. A regression
		// that didn't collapse rotation < ROTATION_EPSILON to identity in BOTH methods would produce different
		// image-coords for a finger touch on a not-actually-rotated image.
		ViewportMath math = standardMath();
		CropState state = new CropState();
	...
	{
		// The convenience overload screenToImagePixel allocates a fresh float[2] per call. Pin so a regression
		// that returned an internal cached buffer would corrupt callers that stash results across pan/zoom
		// updates. Use assertNotSame for reference-identity — identityHashCode is not guaranteed unique per
		// Object.hashCode contract, so two distinct arrays could theoretically hash-collide and falsely pass an
		// identityHashCode-inequality check.
		ViewportMath math = standardMath();
	...
	{
		// Pin that ViewSize.width() / height() are called each time, not cached at construction. A regression
		// that captured dimensions in the constructor would break the layout-then-fitToView flow (View's
		// getWidth/getHeight return 0 before layout, and the production CropEditorView constructs ViewportMath
		// in the field initializer — before measure / layout). Use a mutable FixedViewSize to test this.
	...
	{
		// Edge case: View not yet measured (getWidth/getHeight = 0). At construction default baseScale = 1 and
		// zoom = 1, so scale = 1 (NOT collapsed — the production fitToView guards against zero-width view; the
		// raw imageToScreenX still runs with default state). The conversion formula is `viewW/2 + (ix
		// -viewportX) * scale` → `0 + (100 - 0) * 1 = 100`. Pin: the function must NOT throw / NaN-out, and the
		// result is the deterministic value from the formula at default state.
	...
	{
		// End-to-end: user fits at rotation=0, rotates 30°, pinches out. The clamp's lower bound must drop
		// below 1 here so a sufficiently-large scaleFactor < 1 reduces zoom below 1 — a hardcoded MIN_ZOOM
		// floor of 1 leaves the rotated image extending past the view.
		FixedViewSize size = new FixedViewSize(1000, 1000);
		ViewportMath math = new ViewportMath(size);
	...
	{
		// Zoom factor multiplied by current zoom is clamped to [MIN_ZOOM=1, maxZoom()]. The upper bound is
		// per-image: maxZoom() = MAX_PIXEL_RATIO / baseScale so each source pixel renders at most
		// MAX_PIXEL_RATIO (64) screen pixels. View 400×800 + image 1000×2000 → baseScale = 0.4, so maxZoom = 64
		// / 0.4 = 160. From default zoom=1, a 0.01 factor clamps DOWN to 1; a 1000 factor clamps UP to 160.
		ViewportMath math = standardMath();
		math.fitToView(1000, 2000);
	...
	{
		// The "focus stays under the finger" identity holds when the post-zoom viewport clamp doesn't have to
		// pull the viewport away from the focus. Use a fitToView setup where the focus is at the image-center
		// screen position (200, 400) — the viewport center stays put across the zoom (no clamp interference).
		// Image at center maps to screen-center BEFORE the zoom and AFTER the zoom; image-x of the focus is
		// invariant.
		ViewportMath math = standardMath();
		math.fitToView(1000, 2000);
		...
		math.zoomAt(2f, 200f, 400f, 1000, 2000);
		// Post-zoom: screen (200, 400) should still map to the same image point (the focus didn't move from
		// under the finger).
	...
	public void zoomAtPureOverloadDelegatesFromCropStateOverloadWithNullImage()
	{
		...
		// arithmetic but skips the post-zoom clamp. Verify by checking that zoom landed at the clamped value
		// and the focus identity holds at default scale=1.
		ViewportMath math = standardMath();
		// Default state: baseScale = 1, zoom = 1, viewportX = 0, viewportY = 0.
		math.zoomAt(2f, 200f, 400f, (CropState) null);
		assertEquals("zoom of 2 accepted unclamped (within [1, maxZoom() = 64 at baseScale 1])",
			2f, math.getZoom(), 0f);
	...
	/**
	 * Construct a ViewportMath over the canonical 400x800 FixedViewSize. Covers every test that never reads the
	 * size object again; tests that read or mutate view dimensions keep their explicit arrange blocks.
	 *
	 * @return a fresh ViewportMath at default state over a 400x800 view
	 */
	private static ViewportMath standardMath()
	{
		return new ViewportMath(new FixedViewSize(400, 800));
	}
}
