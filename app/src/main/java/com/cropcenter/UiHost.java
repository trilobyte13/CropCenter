...
interface UiHost extends EditorHost
{
	...
	/**
	 * Currently-active lock-mode preference — select vs move modes track their own preferences, and this returns
	 * whichever one the current editor mode uses.
	 */
	...
	/**
	 * Move-mode lock preference (BOTH / HORIZONTAL / VERTICAL). Exposed separately from getCurrentPref so UiSync's
	 * mode-switch can demote BOTH → VERTICAL when leaving Select mode (BOTH is Select-only).
	 */
	...
	/**
	 * True while UiSync.syncRotationUi is mid-update of the rotation ruler, gating the ruler's own change listener
	 * from re-entering CropState.setRotationDegrees.
	 */
	boolean isRulerUpdating();

	/**
	 * Directly set the move-mode lock preference, used by UiSync to demote Move + BOTH → Move + VERTICAL when
	 * leaving Select mode.
	 */
	void setMoveLockPref(CenterMode pref);

	/**
	 * Raise/clear the ruler-updating flag around a programmatic ruler write so the ruler's own change listener
	 * doesn't fire on the round-trip.
	 */
