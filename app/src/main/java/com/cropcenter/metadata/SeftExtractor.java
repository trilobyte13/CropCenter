...
/**
 * Extracts the Samsung SEFT trailer from a JPEG file.
 * The SEFT trailer is appended after the last JPEG EOI (after gain map if present).
 * Layout: [SEFT data blocks][SEFH directory][4-byte size LE][4-byte "SEFT" magic]
 */
public final class SeftExtractor {
    ...
    public static byte[] extract(byte[] file) {
        if (file == null || file.length < 12) return null;
        int len = file.length;
        ...
        if (trailerStart < 0 || trailerStart >= len) return null;
