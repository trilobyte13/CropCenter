...
public final class GainMapExtractor {
    ...
    public static byte[] extract(byte[] file) {
        if (file == null || file.length < 10) return null;
        ...
        while (off < endBound - 1) {
            ...
            if (m == 0xDA) {
                if (off + 3 >= endBound) break;
                int sosLen = ((file[off+2] & 0xFF) << 8) | (file[off+3] & 0xFF);
                off += 2 + sosLen;

                while (off < endBound - 1) {
                    if ((file[off] & 0xFF) != 0xFF) { off++; continue; }
                    int next = file[off + 1] & 0xFF;
                    if (next == 0xD9) {
                        return extractBetween(file, off + 2, endBound);
                    }
                    if (next == 0x00) { off += 2; continue; }             // byte-stuffed
                    if (next >= 0xD0 && next <= 0xD7) { off += 2; continue; } // restart marker
    ...
    private static byte[] extractBetween(byte[] file, int primaryEnd, int endBound) {
        if (primaryEnd >= endBound || primaryEnd + 1 >= file.length) return null;
        int gmLen = endBound - primaryEnd;
        if (gmLen < 4) return null;
